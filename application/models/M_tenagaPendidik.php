...
class M_tenagaPendidik extends CI_Model {
    function get(){
        return $this->db->get("tbl_tenagapendidik");
    }

    function getWhere($where = NULL,$orderBy = NULL){
        $this->db->select("*");
		$this->db->from("tbl_tenagapendidik");
    ...
    function getWhereStatistic($where = NULL,$orderBy = NULL){
        ...
        $this->db->join("tbl_lembaga", "tbl_lembaga.id = tbl_tenagapendidik.id_lembaga");
    ...
    function insert($data){
        return $this->db->insert("tbl_tenagapendidik",$data);
    }

    function update($data,$params){
        $this->db->where($params);
        return $this->db->update('tbl_tenagapendidik', $data);;
    }

    function delete($params){
        return $this->db->delete("tbl_tenagapendidik",$params);
    }
	...
	function datatable($limit,$start,$search,$paramsSearch,$columnSortName,$typeSort,$selectFilterName,$selectFilterValue){
		$this->db->select("tbl_tenagapendidik.*, tbl_lembaga.nama_lembaga, concat(tbl_tenagapendidik.tempat_lahir,' ',tbl_tenagapendidik.tanggal_lahir) as ttl");
		$this->db->from("tbl_tenagapendidik");
        $this->db->join("tbl_lembaga","tbl_tenagapendidik.id_lembaga = tbl_lembaga.id");
        ...
        if($this->session->userdata("role") == "operator"){
            $this->db->where("tbl_tenagapendidik.id_lembaga",$this->encryption->decrypt(base64_decode($this->session->userdata("id_lembaga"))));
        }
