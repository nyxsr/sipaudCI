...
class Kelola_tenagaPendidik extends CI_Controller {

	function __construct() {
		parent::__construct();
		$this->load->model(['M_tenagaPendidik','M_Lembaga']);
	...
	function index()
	{
		$data['konten'] = "kelola_tenagaPendidik/v_kelola_tenagaPendidik";
        $data['judul'] = "Kelola Tenaga Pendidik";
	...
	function datatable(){
		...
		if($this->session->userdata("role") == "operator"){
			$total = $this->M_tenagaPendidik->totalRow(['id_lembaga' => $this->encryption->decrypt(base64_decode($this->session->userdata("id_lembaga")))]);
		}else{
			$total = $this->M_tenagaPendidik->totalRow();
		}
		$output = array();
		$output['draw']	 = $draw;
		...
		if($search!=""){
			$query = $this->M_tenagaPendidik->datatable($length,$start,$search,"tbl_tenagapendidik.nama",$columnName,$columnSortOrder,$selectFilterName,$selectFilterValue);
			...
			$output['data'][]=array(
				...
				'<div class="row">
					<div class="col-md-6 text-center">
						<a href="javascript:void(0);" class="text-success modalButton"  data-toggle="modal" data-target="#modal" data-type="edit" data-id="'.base64_encode($this->encryption->encrypt($dt['id_tenagaPendidik'])).'"><i class="fa fas fa-pencil"></i></a>
					</div>
					<div class="col-md-6 text-center">
						<a href="javascript:void(0);" onclick="hapus(this.id);" class="text-danger" id="'.base64_encode($this->encryption->encrypt($dt['id_tenagaPendidik'])).'"><i class="fa fas fa-trash"></i></a>
					</div>
				</div>',
			);
			$nomor_urut++;
	...
	public function modalTambah(){
		...
		$this->load->view('kelola_tenagaPendidik/modalAdd',$data);
	}
	...
	public function add(){
		...
		$query				= $this->M_tenagaPendidik->insert($data);
	...
	public function modalEdit(){
		...
		$data['data'] = $this->M_tenagaPendidik->getWhere(["id_tenagaPendidik"=>$this->encryption->decrypt(base64_decode($id))])->row_array();
		$data['dataLembaga'] = $this->M_Lembaga->get()->result_array();
		$this->load->view('kelola_tenagaPendidik/modalEdit',$data);
	}
	...
	function edit(){
		...
		$query 				= $this->M_tenagaPendidik->update($dataArray, $params);
		echo json_encode($query);
	}
	...
	public function delete(){
		...
		$data				= array('id_tenagaPendidik'=>$id);
		$query				= $this->M_tenagaPendidik->delete($data);
		echo json_encode($query);
	}
	...
	public function cek_nip(){
		...
		$where = ["nip"=>$nip];
		if(!empty($id)){
			$where["id_tenagaPendidik !="] = $id;
		}
		$output = $this->M_tenagaPendidik->totalRow($where);
		if($output > 0){
			$output = false;
		}else{
			$output = true;
