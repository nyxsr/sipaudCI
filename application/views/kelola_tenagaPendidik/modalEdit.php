...
<div class="modal-header">
    ...
    <h4 class="modal-title" id="myModalLabel">Ubah Tenaga Pendidik</h4>
</div>
...
function cekNIP(){
    ...
    $.ajax({
        ...
        dataType: "json",
        data: {
            id:"<?= base64_encode($this->encryption->encrypt($data['id_tenagaPendidik'])) ?>",
            nip: nip
        },
...
$('[name="btn_simpan"]').on('click', function() {
    ...
    if (
        ...
        if ($('#inputan_nuptk').hasClass("has-success")) {
            $.ajax({
                type: "POST",
                url: "<?php echo base_url() ?>kelola_tenagaPendidik/edit",
                dataType: "json",
                data: {
                    id:"<?= base64_encode($this->encryption->encrypt($data['id_tenagaPendidik'])) ?>",
                    nip: nip,
