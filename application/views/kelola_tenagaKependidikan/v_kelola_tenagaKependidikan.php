    ...
    <div class="card">
        <div class="card-header card-header-primary">
            <div class="d-inline-flex">
                <h4 class="d-inline-block align-middle" style="margin-top: 0;margin-bottom: 0;padding: 0.375rem 0.75rem;">Daftar Tenaga Kependidikan</h4>
            </div>
...
<script>
    ...
    $(document).ready(function() {
        var lembaga = [];
        $.get( "<?php echo base_url('Kelola_tenagaKependidikan/getLembaga'); ?>", function( responseFetch ) {
        ...
        datatable = $("#datatable").DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "<?php echo base_url('Kelola_tenagaKependidikan/datatable'); ?>",
                type:"POST",
                dataSrc: datatable,
                dataFilter: function ( response ) {
                    var data = JSON.parse(response);
                    data.yadcf_data_8 = lembaga;
            ...
            "columns": [
                ...
                {data : 1, name: 'tbl_tenagapendidik.nip'},
                {data : 2, name: 'tbl_tenagapendidik.nama'},
                {data : 3, name: 'tbl_tenagapendidik.alamat'},
                {data : 4, name: 'tbl_tenagapendidik.no_telp'},
                {data : 5, name: 'tbl_tenagapendidik.pendidikan'},
                {data : 6, name: 'ttl', render: function ( data, type, row ) {
                    return `${data.tempat}, ${data.tanggal}`
                }},
                {data : 7, name: 'tbl_tenagapendidik.jenis_kelamin'},
                {data : 8, name: 'tbl_lembaga.nama_lembaga'},
    ...
    $(document).on("click",".modalButton",function(){
        ...
        if(jenis == "tambah"){
            $("#modal-content").load("<?= base_url('kelola_tenagaKependidikan/modalTambah') ?>")
        }else{
            var id = $(this).attr('data-id');
            $("#modal-content").load("<?= base_url('kelola_tenagaKependidikan/modalEdit') ?>",{id:id})
        }
    });
    ...
    function hapus(id){
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "POST",
                    url: "<?php echo base_url() ?>kelola_tenagaKependidikan/delete",
                    dataType: "json",
                    data: {
                        id: id
                    },
