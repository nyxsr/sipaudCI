...
<div class="modal-header">
    ...
    <h4 class="modal-title" id="myModalLabel">Edit Data Sarana</h4>
</div>
                        ...
                        <form method="POST" id="addForm" action="javascript:void(0);">
                            <div class="row" style="padding-bottom: 0px;">
                                <div class="list col-md-12 dropdown bootstrap-select dropdown" style="width:100% !important;">
                                    ...
                                    <select name="nama_lembaga" class="selectpicker" data-style="select-with-transition" title="Pilih Lembaga" data-size="7" tabindex="-98" <?= ($this->session->userdata('role') == "operator") ?
                                                                                                                                                                                "disabled" : "" ?>>
                                        <?php
                                        foreach ($dataLembaga as $index => $d) :
                                        ?>
                                            <option value="<?= $d['id'] ?>" <?= ($d['id'] == $data['id_lembaga'])
                                                                                ? "selected" : "" ?>>
                                                <?= $d['nama_lembaga'] ?>
                                            </option>
                                        <?php
                            ...
                            <div class="mb-5">
                                ...
                                <div>
                                    <div class="custom-control custom-checkbox d-block">
                                        <input type="checkbox" class="custom-control-input" id="perosotan" name="APEL_perosotan" <?= !empty($data['APEL_perosotan']) ? "checked" : '' ?> />
                                        <label class="custom-control-label" for="perosotan">Perosotan</label>
                                    </div>

                                    <div class="custom-control custom-checkbox d-block">
                                        <input type="checkbox" class="custom-control-input" id="jungkitan" name="APEL_jungkitan" <?= !empty($data['APEL_jungkitan']) ? "checked" : '' ?> />
                                        <label class="custom-control-label" for="jungkitan">Jungkitan</label>
                                    </div>

                                    <div class="custom-control custom-checkbox d-block">
                                        <input type="checkbox" class="custom-control-input" id="ayunan" name="APEL_ayunan" <?= !empty($data['APEL_ayunan']) ? "checked" : '' ?> />
                                        <label class="custom-control-label" for="ayunan">Ayunan</label>
                                    </div>

                                    <div class="custom-control custom-checkbox d-block">
                                        <input type="checkbox" class="custom-control-input" id="bola_dunia" name="APEL_bola_dunia" <?= !empty($data['APEL_bola_dunia']) ? "checked" : '' ?> />
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <div>
                                            <div class="custom-control custom-checkbox d-block">
                                                <input type="checkbox" class="custom-control-input" id="alat_main_sensori_motor" name="APED_sensori_motor" <?= !empty($data['APED_sensori_motor']) ? "checked" : '' ?> />
                                                <label class="custom-control-label" for="alat_main_sensori_motor">Alat
                                                    Main Sensori Motor</label>
                                            </div>

                                            <div class="custom-control custom-checkbox d-block">
                                                <input type="checkbox" class="custom-control-input" id="alat_main_pembangunan" name="APED_pembangunan" <?= !empty($data['APED_pembangunan']) ? "checked" : '' ?> />
                                                <label class="custom-control-label" for="alat_main_pembangunan">Alat
                                                    Main Pembangunan</label>
                                            </div>

                                            <div class="custom-control custom-checkbox d-block">
                                                <input type="checkbox" class="custom-control-input" id="alat_bermain_peran" name="APED_bermain_peran" <?= !empty($data['APED_bermain_peran']) ? "checked" : '' ?> />
                                                <label class="custom-control-label" for="alat_bermain_peran">Alat
                                                    Bermain Peran</label>
                                            </div>

                                            <div class="custom-control custom-checkbox d-block">
                                                <input type="checkbox" class="custom-control-input" id="imtaq" name="APED_imtaq" <?= !empty($data['APED_imtaq']) ? "checked" : '' ?> />
                                ...
                                <button class="btn btn-info pull-right" id="btn_simpan" name="btn_simpan">Ubah</button>
...
<script>
    ...
    $('[name="btn_simpan"]').on('click', function() {
        ...
        if (
            ...
            $.ajax({
                type: "POST",
                url: "<?php echo base_url() ?>kelola_sarana/edit",
                dataType: "json",
                data: {
                    id: "<?= base64_encode($this->encryption->encrypt($data['id_sarana'])) ?>",
