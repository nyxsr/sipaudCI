...
<div class="container-fluid">
        ...
        <div class="card">
            <div class="card-header card-header-primary">
                <div class="d-inline-flex">
                    <h4 class="d-inline-block align-middle" style="margin-top: 0;margin-bottom: 0;padding: 0.375rem 0.75rem;">Daftar Bantuan</h4>
                </div>
                <div class="cold-md-4 align-middle <?= ($this->session->userdata('role') == "operator") ? "d-none" : "d-inline-flex" ?>" style="float:right;">
                    <div id="external_filter_container_6" style="width:100% !important;">
                    </div>
                </div>
            </div>
            <div class="card-body">
    ...
    <script>
        ...
        $(document).ready(function() {
            var lembaga = [];
            $.get("<?php echo base_url('kelola_bantuan/getLembaga'); ?>",
            ...
            data = $("#datatable").DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "<?php echo base_url('Kelola_Bantuan/datatable'); ?>",
                    type: "POST",
                    dataSrc: data,
                ...
                "columns": [{
                    ...
                    {
                        data: 1,
                        name: 'tbl_bantuan.tahun'
                    },
                    {
                        data: 2,
                        name: 'tbl_bantuan.jenis_bantuan'
                    },
                    {
                        data: 3,
                        name: 'tbl_bantuan.sumber_bantuan'
                    },
                    {
                        data: 4,
                        name: 'tbl_bantuan.keterangan_bantuan'
                    },
                    {
                        data: 5,
                        name: 'tbl_bantuan.jumlah'
                    },
                    {
                        data: 6,
        ...
        $(document).on("click", ".modalButton", function() {
            ...
            if (jenis == "tambah") {
                $("#modal-content").load("<?= base_url('kelola_bantuan/modalTambah') ?>")
            } else {
                var id = $(this).attr('data-id');
                $("#modal-content").load("<?= base_url('kelola_bantuan/modalEdit') ?>", {
                    id: id
                })
            }
        });
        ...
        function hapus(id) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: "POST",
                        url: "<?php echo base_url() ?>kelola_bantuan/delete",
                        dataType: "json",
                        data: {
                            id: id
                        },
