    ...
    <div class="card">
        <div class="card-header card-header-primary">
            <div class="d-inline-flex">
                <h4 class="d-inline-block align-middle" style="margin-top: 0;margin-bottom: 0;padding: 0.375rem 0.75rem;">Daftar Siswa</h4>
            </div>
            <div class="cold-md-4 align-middle <?= ($this->session->userdata('role') == "operator") ? "d-none" : "d-inline-flex" ?>" style="float:right;">
                <div id="external_filter_container_10" style="width:100% !important;">
                </div>
            </div>
        </div>
        <div class="card-body">
...
<script>
    ...
    $(document).ready(function() {
        var lembaga = [];
        $.get( "<?php echo base_url('Kelola_Siswa/getLembaga'); ?>", function( responseFetch ) {
        ...
        datatable = $("#datatable").DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "<?php echo base_url('Kelola_Siswa/datatable'); ?>",
                type:"POST",
                dataSrc: datatable,
            ...
            "columns": [
                ...
                {data : 1, name: 'tbl_siswa.nama'},
                {data : 2, name: 'tbl_siswa.NISN'},
                {data : 3, name: 'tbl_siswa.jenis_kelamin'},
                {data : 4, name: 'ttl', render: function ( data, type, row ) {
                    return `${data.tempat}, ${data.tanggal}`
                }},
                {data : 5, name: 'tbl_siswa.alamat'},
                {data : 6, name: 'tbl_siswa.nama_ortu'},
                {data : 7, name: 'tbl_siswa.no_telp_ortu'},
                {data : 8, name: 'tbl_siswa.pekerjaan_ortu'},
                {data : 9, name: 'tbl_siswa.usia_adik'},
                {data :10, name: 'tbl_lembaga.nama_lembaga'},
                {data :11, name: 'tbl_siswa.tanggal_keluar'},
    ...
    $(document).on("click",".modalButton",function(){
        ...
        if(jenis == "tambah"){
            $("#modal-content").load("<?= base_url('kelola_siswa/modalTambah') ?>")
        }else{
            var id = $(this).attr('data-id');
            $("#modal-content").load("<?= base_url('kelola_siswa/modalEdit') ?>",{id:id})
        }
    });
    ...
    function hapus(id){
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "POST",
                    url: "<?php echo base_url() ?>kelola_siswa/delete",
                    dataType: "json",
                    data: {
                        id: id
                    },
